...
function getNetSalary(grossSalary) {
    ...
    function getSocialSecurityTax(gs1) {
        
        let taxA = percent(gs1, 8);
        let taxB = percent(gs1, 9);
        let taxC = percent(gs1, 11);
        let taxValueA = 570.88;

        if (gs1 <= 1556.94) {
            socialSecurityTax = taxA.toFixed(2);
        } else if (gs1 >= 1556.95 && gs1 <= 2594.92) {
            socialSecurityTax = taxB.toFixed(2);
        } else if (gs1 >= 2594.93 && gs1 <= 5198.82) {
            socialSecurityTax = taxC.toFixed(2);
        } else {
            socialSecurityTax = taxValueA;
        }
        console.log("social security tax: " + socialSecurityTax);
    }
    getSocialSecurityTax(grossSalary);

    function getIncomeTax(gs2) {
        
    }

}

getNetSalary(1556.94);